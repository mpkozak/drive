...
///////////////////////////////////
// FUNCTION MAKE NEW GAME OBJECT //
///////////////////////////////////
  function newGameObject(gamePlane, addClass, imgSrc) {
    let div = document.createElement('div');
    div.style.position = 'absolute';
...
/////////////////////////
function makeTrees() {
  moveBox(newGameObject(p7, 'tree', "url('img/tree.png')"), 6, -7, 4, 8);
  moveBox(newGameObject(p7, 'tree', "url('img/tree.png')"), 10, 19, 4, 8);
};
/////////////////////////


/////////////////////////////////
// MOVE ARBITRARY BOX FUNCTION //
/////////////////////////////////
function moveBox(element, startL, endL, endW, endH) {
  let box = element;
  box.style.transitionTimingFunction = 'linear';

  let startLeft = startL * w;
  let startTop = 2 * h;
  let startWidth = 0;
  let startHeight = 0;
  ...
  let factor = 50;
  let t = 0;
  let moveIt = setTimeout(function move() {
    if (t < factor) {
      let rate = (Math.pow(t, 5) / Math.pow((factor - 1), 5));
      // box.style.transition = 'all ' + (200000 / speed) / factor + 'ms linear';
      box.style.transitionDuration = (200000 / speed) / factor + 'ms';
      // box.style.transitionTimingFunction = 'linear';
    ...
    moveIt = setTimeout(move, (200000 / speed) / factor)
  }, (200000 / speed) / factor);
};
/////////////////////////////////
...
let treesToMake = 20;
// let treeSpeed = speed / 2000;
// let treeTime = 100 / treeSpeed;
// let treeInt = treeLifeSpan / (treesToMake / 2);
// let treeLifeSpan = 200000 / speed;
let treeInt = (200000 / speed) / (treesToMake / 2);
...
let moveTrees = setTimeout(function move() {
  ...
  treeInt = (200000 / speed) / (treesToMake / 2);

  moveTrees = setTimeout(move, treeInt);
      treesMade = document.querySelectorAll('.tree').length;
      console.log('speed ' + speed.toFixed(2), speedChange, 'treeSpeed ' + treeSpeed.toFixed(2), 'treeTime ' + treeTime.toFixed(2), 'treeInt ' + treeInt.toFixed(2), 'treesMade ' + treesMade)
}, treeInt);
