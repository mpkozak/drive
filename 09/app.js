...
// Function Move + Scale Dynamic Object Then Remove
    function moveBox(element, t) {
      ...
      // let distance = distanceOld + speed * (t - box.classList[2]);
            let distance = distanceOld + thisSpeed * (t - box.classList[2]);
      ...
      //   box.remove();
      // };
      let newTop = parseInt(box.style.top.replace(/px/g, ''));
      if (newTop >= 7 * h && box.classList[0] === 'enemy') {
        box.parentNode.removeChild(box);
        p4.appendChild(box);
      } else if (newTop < 7 * h && box.classList[0] === 'enemy') {
        box.parentNode.removeChild(box);
        p6.appendChild(box);
      } else if (newTop > 9 * h && box.classList[0] === 'tree') {
        box.remove();
      } else if (newTop > 12 * h && box.classList[0] === 'enemy') {
        box.remove();
      }
    };
